...
function loadMenuTable() {
	$.get(urlHolder.menuLinks, function(response) {
		
 		$('#tableLinks').find('tbody').remove();
 		...
 		for (var i=0; i<response.menuItems.length; i++) {
			var row = '<tr>';
			row += '<td><a href="' + response.menuItems[i].url + '" target="_blank">' + response.menuItems[i].name + '</a></td>';
			row += '</tr>';
	 		$('#tableLinks').append(row);
 		}
 		
 		$('#tableLinks').data('model', response.users);
		toggleLogin('hide'); ;
 	});
}
...
function authenticated() {
	$.get(urlHolder.authenticatedLink, function(response) {
		var test = response.authenticated;
		authHolder.value = test;
 	});
}
